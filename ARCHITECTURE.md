...
实现RSS内容生成、存储和访问功能，支持按关键词订阅热点新闻。
...
**关键方法**:
- `generate_rss()`: 生成完整RSS Feed
- `generate_rss_by_keyword()`: 按关键词生成RSS
- `validate_rss()`: 验证RSS格式正确性

**文件位置**: rss/generator.py
...
**关键方法**:
- `generate_rss_feed()`: 生成RSS Feed
- `generate_and_save_rss()`: 生成并保存RSS
- `get_rss_by_keyword()`: 获取特定关键词的RSS
...
**关键方法**:
- `save_rss_feed()`: 保存RSS Feed
- `save_multiple_feeds()`: 保存多个关键词的RSS
- `list_available_keywords()`: 列出可用的RSS关键词

**文件位置**: rss/storage.py

### 4. MCP服务器 (mcp_server/server.py)

基于 FastMCP 2.0 框架,提供 18 个 AI 工具。
...
5. **get_rss_subscriptions** - 获取RSS订阅列表
   - 输出: 所有可用的RSS订阅信息

6. **get_rss_by_keyword** - 获取特定关键词的RSS
   - 输入: 关键词 (如 "AI" 或 "all")
   - 输出: 符合RSS 2.0标准的XML内容

7. **get_rss_statistics** - 获取RSS统计信息
   - 输出: RSS订阅数量、总大小等统计数据

8. **get_rss_history** - 获取RSS历史记录
   - 输入: 关键词
   - 输出: 历史RSS文件信息
